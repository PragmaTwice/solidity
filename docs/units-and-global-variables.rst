...
.. note::
    ...
    However, the ``--evm-version`` setting used when compiling the contract has no bearing on it.

    Additionally, prior to version 0.5.0, there was a function called ``suicide`` with the same
